...
:all
echo make all
%CYGWIN_PATH%\bin\bash.exe -login -c "cd ${TC_DIR}/project/tcd22x0_EVM003/make;make -s dep AWK_PIPE=1"
@goto end

:rebuild
echo rebuild all
%CYGWIN_PATH%\bin\bash.exe -login -c "cd ${TC_DIR}/project/tcd22x0_EVM003/make;make clean AWK_PIPE=1;make install AWK_PIPE=1"
@goto end

:clean
echo make clean
%CYGWIN_PATH%\bin\bash.exe -login -c "cd ${TC_DIR}/project/tcd22x0_EVM003/make;make clean"
@goto end
:end