...
#if 0 //zluo, I want to put the routine inside ROM
//----------------------------------------------------------------------------
...
int  flash_erase_block_nonblock(void* block, unsigned int size);
    __attribute__ ((section (".2ram.flash_erase_block_nonblock")));

int  flash_erase_block_stat_nonblock(void* block, unsigned int size);
    __attribute__ ((section (".2ram.flash_erase_block_stat_nonblock")));
...
#ifdef _TC_FLASH_ADDITIONS
static int  _flash_erase_block_nonblock(void* block, unsigned int size) 
    __attribute__ ((section (".2ram._flash_erase_block_nonblock")));

static int  _flash_erase_block_stat_nonblock(void* block, unsigned int size) 
    __attribute__ ((section (".2ram._flash_erase_block_stat_nonblock")));
#endif //_TC_FLASH_ADDITIONS
...
static void _flash_query(void* data);
static int  _flash_erase_block(void* block, unsigned int size);
static int  _flash_program_buf(void* addr, void* data, int len);
...
#ifdef _TC_FLASH_ADDITIONS
static int
_flash_erase_block_nonblock(void* block, unsigned int size)
{
    volatile flash_data_t* ROM, *BANK;
    volatile flash_data_t* b_p = (flash_data_t*) block;
    volatile flash_data_t *b_v;
    ...
    *f_s0 = FLASH_Reset;
    ...
    if (FLASH_unlocked != state)
	{
//	    diag_printf("flash error state:%i\n", state);
        return FLASH_ERR_PROTECT;
	}
#endif
    ...
    *b_v = FLASH_Block_Erase;
    ...
    while (true) {
        state = *b_v;
        if ((state & FLASH_Sector_Erase_Timer) == FLASH_Sector_Erase_Timer) break;
...
int
flash_erase_block_nonblock(void* block, unsigned int size)
{
    int ret, cache_on;
    HAL_DCACHE_IS_ENABLED(cache_on);
    if (cache_on) {
    ...
    ret = _flash_erase_block_nonblock(block, size);
    if (cache_on) {
        HAL_DCACHE_ENABLE();
    }
    return ret;
...
// Dette her skal ind i status funktionen.
static int
_flash_erase_block_stat_nonblock(void* block, unsigned int size)
{
    volatile flash_data_t* ROM, *BANK;
    volatile flash_data_t* b_p = (flash_data_t*) block;
    volatile flash_data_t *b_v;
    ...
    *((unsigned long*)0xbf000004) = 0x5678; //hack, kick the wd
	...
	else res = FLASH_ERR_ERASE_BUSY;

    return res;
}

int
flash_erase_block_stat_nonblock(void* block, unsigned int size)
{
    int ret, cache_on;
    HAL_DCACHE_IS_ENABLED(cache_on);
    if (cache_on) {
    ...
    ret = _flash_erase_block_stat_nonblock(block, size);
    if (cache_on) {
        HAL_DCACHE_ENABLE();
    }
    return ret;
