...
#define _TC_FLASH_ADDITIONS
//#undef _TC_FLASH_ADDITIONS

#ifdef _TC_FLASH_ADDITIONS
// flash erase and program callback typedef
typedef void (*TC_FLASH_CB)(void);

// note: the flash may not be accessed during the non blocking flash erase functions
// flash erase - repeatedly calls callback while waiting for erase to be finished
externC int flash_erase_with_cb(void *base, int len, void **err_address, TC_FLASH_CB callback);
// flash erase non-blocking - start erase on one sector
//   returns FLASH_ERR_OK: done, FLASH_ERR_MORE_SECTORS: more sectors to be erased, o.w. error
//   if FLASH_ERR_MORE_SECTORS then addr_remain and len_remain contains valid values
externC int flash_erase_nonblock(void *base, int len, void **err_address, int *addr_remain, int *len_remain);
// flash erase status - get erase status 
//   returns FLASH_ERR_OK: done, FLASH_ERR_ERASE_BUSY: erasing, o.w. error
externC int flash_erase_stat_nonblock(void *base, int len, void **err_address);

// flash program - repeatedly calls callback after programming each sector
externC int flash_program_with_cb(void *flash_base, void *ram_base, int len, void **err_address, TC_FLASH_CB callback); 
...
#ifdef _TC_FLASH_ADDITIONS
#define FLASH_ERR_ERASE_BUSY      0x0f  // Erase not finished
#define FLASH_ERR_MORE_SECTORS    0x10  // Erase area contains more than one sector
#endif //_TC_FLASH_ADDITIONS
