                       ...
                       cyg_uint32* len)
{
    ...
    case CYG_IO_GET_CONFIG_FLASH_ERASE_NONBLOCK:
	{
        if ( *len != sizeof( cyg_io_flash_getconfig_erase_t ) )
             return -EINVAL;
        {
            ...
            e->flasherr = flash_erase_nonblock( startpos, e->len, e->err_address, NULL, NULL );
    ...
    case CYG_IO_GET_CONFIG_FLASH_STATUS_NONBLOCK:
	{
        if ( *len != sizeof( cyg_io_flash_getconfig_erase_t ) )
             return -EINVAL;
        {
            ...
            e->flasherr = flash_erase_stat_nonblock( startpos, e->len, e->err_address );
		}
		return ENOERR;
	}
