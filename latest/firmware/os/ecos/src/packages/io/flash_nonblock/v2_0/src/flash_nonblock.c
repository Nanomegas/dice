...
#include <pkgconf/io_flash_nonblock.h>
...
#ifdef _TC_FLASH_ADDITIONS
int
flash_erase_nonblock(void *addr, int len, void **err_addr, int *addr_remain, int *len_remain)
{
    unsigned short *block, *end_addr;
    int stat = 0;
    typedef int code_fun(unsigned short *, unsigned int);
    code_fun *_flash_erase_block_nonblock;
    int d_cache, i_cache;
	int count = 0;

	if (addr_remain && len_remain)
	{
		*addr_remain = 0;
		*len_remain = 0;
	}
...
#ifdef CYGHWR_IO_FLASH_DEVICE_NOT_IN_RAM
    {
        extern char flash_erase_block_nonblock[], flash_erase_block_nonblock_end[];
        CYG_ADDRESS code_len;

        // Copy 'erase' code to RAM for execution
        code_len = (CYG_ADDRESS)&flash_erase_block_nonblock_end - 
				   (CYG_ADDRESS)&flash_erase_block_nonblock;
        _flash_erase_block_nonblock = (code_fun *)flash_info.work_space;
        memcpy(_flash_erase_block_nonblock, &flash_erase_block_nonblock, code_len);
    }
#else
    {
        externC code_fun flash_erase_block_nonblock;
        _flash_erase_block_nonblock = (code_fun*) __anonymizer(&flash_erase_block_nonblock);
    }
#endif
    ...
    while (block < end_addr) {
        ...
        if (!erased) {
			if (count != 0)
			{
				if (addr_remain && len_remain)
				{
					*addr_remain = (int) block;
					*len_remain = (int) (len - flash_info.block_size);
				}
				stat = FLASH_ERR_MORE_SECTORS;
	            *err_addr = (void *)block;
				break;
			}
            stat = (*_flash_erase_block_nonblock)(block, flash_info.block_size);
            stat = flash_hwr_map_error(stat);
			count++;
        }
        if (stat) {
...
int
flash_erase_stat_nonblock(void *addr, int len, void **err_addr)
{
    unsigned short *block, *end_addr;
    int stat = 0;
    typedef int code_fun(unsigned short *, unsigned int);
    code_fun *_flash_erase_block_stat_nonblock;
    int d_cache, i_cache;
...
#ifdef CYGHWR_IO_FLASH_DEVICE_NOT_IN_RAM
    {
        extern char flash_erase_block_stat_nonblock[], flash_erase_block_stat_nonblock_end[];
        CYG_ADDRESS code_len;

        // Copy 'erase' code to RAM for execution
        code_len = (CYG_ADDRESS)&flash_erase_block_stat_nonblock_end - 
				   (CYG_ADDRESS)&flash_erase_block_stat_nonblock;
        _flash_erase_block_stat_nonblock = (code_fun *)flash_info.work_space;
        memcpy(_flash_erase_block_stat_nonblock, &flash_erase_block_stat_nonblock, code_len);
    }
#else
    {
        externC code_fun flash_erase_block_stat_nonblock;
        _flash_erase_block_stat_nonblock = (code_fun*) __anonymizer(&flash_erase_block_stat_nonblock);
    }
#endif
    ...
    (*flash_info.pf)("... check status from %p-%p: ", (void*)block, (void*)end_addr);

    HAL_FLASH_CACHES_OFF(d_cache, i_cache);
    FLASH_Enable(block, end_addr);

    stat = (*_flash_erase_block_stat_nonblock)(block, flash_info.block_size);
...
int
flash_erase_with_cb(void *base, int len, void **err_address, TC_FLASH_CB callback)
{
    int			stat;
	int			addr_remain;
	int			len_remain;
	void		*address = base;
	int			length = len;
	bool		sectors_remain = false;		

	for (;;)
	{
		stat = flash_erase_nonblock(address, length, err_address, &addr_remain, &len_remain);
		if (stat != FLASH_ERR_OK && stat != FLASH_ERR_MORE_SECTORS) break;
		sectors_remain = (stat == FLASH_ERR_MORE_SECTORS);
		for (;;)
		{
			if (callback) (* callback)();
			stat = flash_erase_stat_nonblock(address, length, err_address);
			if (stat != FLASH_ERR_ERASE_BUSY) break;
		}
		if (stat != FLASH_ERR_OK) break;
		if (sectors_remain == false) break;
		address = (void *) addr_remain;
		length = len_remain;
	}
...
{
	return flash_program_with_cb(_addr, _data, len, err_addr, NULL);
}

int flash_program_with_cb(void *_addr, void *_data, int len, void **err_addr, TC_FLASH_CB callback)
#else //_TC_FLASH_ADDITIONS
int flash_program(void *_addr, void *_data, int len, void **err_addr)
#endif //_TC_FLASH_ADDITIONS
{
    int stat = 0;
    ...
    while (len > 0) {
        ...
        if (0 == stat) // Claims to be OK
	      ...
	      (*flash_info.pf)("verification failed, %x\n", memcmp(addr, data, size));
                stat = 0x0BAD;
                (*flash_info.pf)("V");
            }
#endif
...
#ifdef _TC_FLASH_ADDITIONS
	case FLASH_ERR_ERASE_BUSY:
        return "Flash is busy erasing";
	case FLASH_ERR_MORE_SECTORS:
        return "Erase area contains more sectors";
#endif //_TC_FLASH_ADDITIONS
    default:
